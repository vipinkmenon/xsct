proc create_ipi_design { offsetfile design_name } {
	...
	# Create instance: imageSizeConverter_0, and set properties
	set imageSizeConverter_0 [ create_bd_cell -type ip -vlnv xilinx.com:user:imageSizeConverter:1.0 imageSizeConverter_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI ] [get_bd_intf_pins imageSizeConverter_0/S00_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/ACLK] [get_bd_pins imageSizeConverter_0/S00_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/ARESETN] [get_bd_pins imageSizeConverter_0/S00_AXI_ARESETN]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/imageSizeConverter_v1_0_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef imageSizeConverter_v1_0_tb_include_vh_"
	puts $fp "`define imageSizeConverter_v1_0_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:imageSizeConverter:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/imageSizeConverter_v1_0_tb.sv
...
for { set i 1 } { 1 } { incr i } {
	set design_name "imageSizeConverter_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 imageSizeConverter_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top imageSizeConverter_v1_0_tb [get_filesets sim_1]
